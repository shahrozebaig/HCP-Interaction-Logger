...
from app.core import database   # ✔ FIX: import module, not db variable
...
async def create_interaction(payload: dict) -> dict:
    ...
    res = await database.db.interactions.insert_one(payload)   # ✔ FIXED
...
async def get_interaction(interaction_id: str) -> Optional[dict]:
    doc = await database.db.interactions.find_one(                   # ✔ FIXED
...
async def list_interactions_for_hcp(hcp_id: str, limit: int = 20) -> List[dict]:
    cursor = (
        database.db.interactions                                     # ✔ FIXED
        .find({"hcpId": hcp_id})
        .sort("date", -1)
        .limit(limit)
    )
